...
NewGameDlg::NewGameDlg(UI::LayoutManager &manager, TextureManager &texman, FileSystem::IFileSystem &fs, ShellConfig &conf, UI::ConsoleBuffer &logger, LangCache &lang)
...
std::string NewGameDlg::GetSelectedMapName()
{
    int index = _maps->GetList()->GetCurSel();
    if( -1 != index )
    {
        return _maps->GetData()->GetItemText(index, 0);
    }
