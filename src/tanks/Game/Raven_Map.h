...
private:
  ...
  void AddWall(std::fstream& in);
  void AddSpawnPoint(std::fstream& in);
  void AddHealth_Giver(std::fstream& in);
  void AddWeapon_Giver(int type_of_weapon, std::fstream& in);
  void AddDoor(std::fstream& in);
  void AddDoorTrigger(std::fstream& in);

  void Clear();
  
public:
  
  Raven_Map(int width, int heigt, TextureManager& texManager);
  Raven_Map(TextureManager& texManager);
  ~Raven_Map();

  void Render(DrawingContext& dc, bool editor);

  //loads an environment from a file
  bool LoadMap(std::fstream& in);
  bool SaveMap(std::fstream& out) const;
