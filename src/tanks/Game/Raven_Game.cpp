...
Raven_Game::Raven_Game(int width, int height, TextureManager& texManager)
	: m_pSelectedBot(NULL),
	m_bPaused(false),
	m_bRemoveABot(false),
	m_pMap(new Raven_Map(width, height, texManager)),
	m_pPathManager(NULL),
	m_pGraveMarkers(NULL),
	m_texManager(texManager)
{
	//TODO:: m_pMap->RebuildGraph();
}

//----------------------------- ctor ------------------------------------------
//-----------------------------------------------------------------------------
Raven_Game::Raven_Game(TextureManager& texManager)
						: m_pSelectedBot(NULL),
                         m_bPaused(false),
                         m_bRemoveABot(false),
                         m_pMap(NULL),
...
//-----------------------------------------------------------------------------
bool Raven_Game::LoadMap(std::fstream& stream)
{  
...
bool Raven_Game::SaveMap(std::fstream& stream) const
{
	return m_pMap->SaveMap(stream);
}
...
Raven_Game::GetPosOfClosestSwitch(Vector2D botPos, unsigned int doorID)const
{
  ...
  {
	  GameEntityBase* trig = EntityMgr->GetEntityFromID(*it);
