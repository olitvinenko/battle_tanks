...
public:
  ...
  Raven_Game(int width, int height, TextureManager&	texManager);
  Raven_Game(TextureManager& texManager);
  ~Raven_Game();
  ...
  bool LoadMap(std::fstream& stream);
  bool SaveMap(std::fstream& stream) const;
  ...
  void  TagRaven_BotsWithinViewRange(GameEntityBase* pRaven_Bot, double range) {TagNeighbors(pRaven_Bot, m_Bots, range);}  
};
