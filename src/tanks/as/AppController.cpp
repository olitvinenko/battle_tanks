...
AppController::AppController(FileSystem::IFileSystem &fs)
    : _fs(fs)
{
}
...
void AppController::NewGameDM(AppState &appState, const std::string &mapName, const DMSettings &settings)
{
    ...
    std::shared_ptr<FileSystem::Stream> stream = _fs.Open(path)->AsStream();
    std::unique_ptr<GameContext> gc(new GameContext(*stream, settings, -1, -1));
