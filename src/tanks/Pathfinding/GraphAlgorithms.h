...
void Graph_SearchDijkstra<graph_type>::Search()
{
	...
	p.push(std::make_pair(0, m_iSource));
	...
	while (!p.empty())
	{
		...
		int NextClosestNode = p.top().second;
		p.pop();
			...
			pE = ConstEdgeItr.next())
		{
			...
			if (m_SearchFrontier[pE->To()] == 0)
			{
				...
				p.push(std::make_pair(NewCost, pE->To()));
...
void Graph_SearchAStar<graph_type, heuristic>::Search()
{
	...
	//put the source node on the queue
	pq.push(std::make_pair(0, m_iSource));
	...
	while (!pq.empty())
	{
			...
			pE = ConstEdgeItr.next())
		{
			...
			if (m_SearchFrontier[pE->To()] == NULL)
			{
				...
				//pq.insert(pE->To());
				pq.push(std::make_pair(GCost + HCost, pE->To()));
