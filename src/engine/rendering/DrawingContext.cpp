...
DrawingContext::DrawingContext(const TextureManager &tm, IRender* render, unsigned int width, unsigned int height)
    ...
    , _mode(UNDEFINED)
{
    _transformStack.push({ Vector2{}, 255 });
    _viewport.left = 0;
    _viewport.top = 0;
...
void DrawingContext::PushTransform(Vector2 offset, float opacityCombined)
{
...
void DrawingContext::DrawBitmapText(Vector2 origin, float scale, size_t tex, Color color, const std::string &str, AlignTextKind align)
{
    color = ApplyOpacity(color, _transformStack.top().opacity);
    ...
    Vector2 pxCharSize = Vec2dFloor(Vector2{ lt.pxFrameWidth, lt.pxFrameHeight } * scale);
